...
export const boardReducer = (state: BoardState, action: BoardAction): BoardState => {
  ...
  switch (action.type) {
    ...
    case ActionType.MOVE_TASK: {
      ...
      // Si la tarea se suelta en la misma lista de la que salió,
      ...
      const updatedBoards = state.boards.map(board => {
        ...
        // Si por alguna razón la tarea no se encontró, devolvemos el tablero
        ...
        // Volvemos a usar `map` sobre el array de listas que ya no contiene la tarea 
        const listsWithTaskAdded = listsWithTaskRemoved.map(list => {
          ...
          // Si no es la lista de destino, la devolvemos sin cambios.
          return list;
        });
